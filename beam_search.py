...
def prefix_beam_search(alphabet, blank_index, ctc, keywords, beam_width=16, beta=1.02, min_keyword_score=.001, max_gap=25, clip_char_prob=.01):
    trie = CharTrie()
    ...
    for t in range(ctc.shape[0]):
        ...
        A_next = Pb[t] + Pnb[t]
        sorted_items = sorted(A_next.items(), key=lambda x: x[1] * (beta ** len(x[0])), reverse=True)
        A_prev = []
        ...
        for prefix, prob in sorted_items:
            ...
            # 1, 3
                ...
                if len(result[prefix]):
                    if (t * .02 - result[prefix][-1]['end'] < max_gap * .02):
                        if result[prefix][-1]['score'] < score:
                            result[prefix].pop()
                            result[prefix].append({'end': t * .02, 'score': score})
                    else:
                        result[prefix].append({'end': t * .02, 'score': score})
                else:
                    result[prefix].append({'end': t * .02, 'score': score})
